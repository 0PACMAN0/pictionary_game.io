...
class TopBar(object):
    ...
    def draw(self, win):
        ...
        win.blit(txt, (self.x + 10, self.y + self.height / 2 - txt.get_height() / 2))
        pygame.draw.circle(win, (0, 0, 0), (self.x + self.width - 50, self.y + self.height / 2), 40,
                           self.BORDER_THICKNESS)
        timer = self.round_font.render(str(self.time),1,(0,0,0))
        win.blit(timer,(self.x + self.width - 50 - (timer.get_width()/2), self.y + round(self.height / 2) - (timer.get_height()/2)))
